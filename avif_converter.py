...
class AVIFConverter:
    def __init__(self):
        ...
        self.preserve_metadata = tk.BooleanVar(value=True)  # 默认保留元数据和时间戳
    ...
    def setup_ui(self):
        ...
        ttk.Checkbutton(options_frame2, text="保留原图元数据和时间戳", variable=self.preserve_metadata).pack(side=tk.LEFT, padx=(0, 20))
    ...
    def start_conversion(self):
        ...
        if not self.ffmpeg_path:
            messagebox.showerror("错误", "未找到 FFmpeg，请确保已 安装并添加到 PATH")
            return
    ...
    def copy_file_timestamps(self, src_path, dst_path):
        """
        将 src_path 的创建时间、修改时间、访问时间复制到 dst_path
        """
        try:
            # 1. 复制修改时间和访问时间（通用方法）
            ...
            # 2. 复制 Windows 创建时间（使用 win32file）
            try:
                # 获取源文件的创建时间（FILETIME 格式）
                ...
                win32file.SetFileTime(handle_dst, ctime, atime, mtime)
                win32file.CloseHandle(handle_dst)
            except Exception as e:
                print(f"复制创建时间失败（可能权限不足）: {e}")
        except Exception as e:
            print(f"复制时间戳失败: {e}")
        ...
        try:
            ...
            if process.returncode == 0:
                ...
                if os.path.exists(output_path):
                    ...
                    # 保留元数据与时间戳
                    if self.preserve_metadata.get():
                        self.copy_file_timestamps(input_path, output_path)
    ...
    def load_config(self):
        ...
        try:
            if os.path.exists('avif_config.json'):
                ...
                self.preserve_metadata.set(config.get('preserve_metadata', True))
